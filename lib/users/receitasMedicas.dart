...
class ReceitaMedica extends StatefulWidget {
  const ReceitaMedica({Key? key}) : super(key: key);
...
class _ReceitaState extends State<ReceitaMedica> {
      ...
      child: Scaffold(
        appBar: AppBar(
          ...
          bottom: const TabBar(
            ...
            tabs: [
              Tab(icon: Icon(Icons.list), text: 'Receitas médicas:'),
            ],
          ),
        ),
        body: TabBarView(
    ...
    builder: (context, snapshot) {
      ...
      return ListView.builder(
          ...
          itemBuilder: (context, index) {
            ...
            var data = exame['dataConsulta'].toDate();
            var dataFormatada = formatarData(data);
            var tipo = exame['tipoConsulta'];
        ...
        child: Form(
          child: Column(
            ...
            children: [
              ...
              const Text('Abrir  receita:', textAlign: TextAlign.center),
              const SizedBox(height: 10),
              ElevatedButton(
                onPressed: () {
                  Navigator.push(
                ...
                child: ElevatedButton.icon(
                  onPressed: () async {
                    Navigator.push(
                      context,
                      MaterialPageRoute(
                        builder: (context) => const ReceitaMedica(),
                      ),
                    );
                  },
